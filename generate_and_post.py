...
ASSORTMENT = (
    ...
    "гіпсокартон та комплектуючі (профілі, кріплення); штукатурка гіпсова; "
    "цементно-піщана суміш; шпаклівки; клей для гіпсокартону, плиточний, для газоблоку, монтажний; "
    "вата фасадна та внутрішня; пінопласт звичайний та екструдований; "
    "металопрофіль та металочерепиця; геотекстиль; фарба-емульсія, емаль, грунтовка, пластифікатори; "
    "цемент ПЦ-400 і ПЦ-500 (франківський та миколаївський); пісок, щебінь, відсів; "
    "піна монтажна, герметик, рідкі цвяхи; стрейч, рукавиці, саморізи, дюбелі"
)
...
STAGE_INSTRUCTIONS = {
    "залучення": ("Тип: ЗАЛУЧЕННЯ. Корисна порада або розвінчання міфу в межах теми «{theme}». "
    ...
    "довіра": ("Тип: ДОВІРА. Поясни в межах «{theme}», що для якої задачі (види, на що звертати увагу, "
               "помилки). НЕ протиставляй товари різного призначення як конкурентів. Технічно коректно."),
...
ACCURACY = (
    "\n\nТОЧНІСТЬ (обовʼязково):\n"
    "- Будматеріали — технічно точно. НЕ плутай призначення товарів і не протиставляй різнопризначені.\n"
    "- НЕ вигадуй характеристик, марок, цифр, ДСТУ, властивостей. Не знаєш точно — формулюй загально.\n"
    "- Поради мають бути практично коректними й безпечними. Сумнівне — прибери.\n"
)
...
HOOK = ("\nГАЧОК: перший рядок чіпляє за секунду — конкретна користь, помилка або питання, "
        "а не визначення.\n")
...
def generate_video(theme):
    """ГОЛОВНЕ: відео з готовим розмовним текстом під камеру + зйомка телефоном."""
    prompt = (
        f"Ти — власник магазину будматеріалів «{SHOP_NAME}» ({CITY}). Асортимент: {ASSORTMENT}. "
        f"Згадуй лише реальні товари. Тема: «{theme}». Зроби ідею короткого ВЕРТИКАЛЬНОГО відео "
        "(Reels 9:16, 20-45 сек) — це ГОЛОВНИЙ контент дня."
        + ACCURACY + HOOK +
        "\nФормат (звичайний текст, без markdown):\n"
        "🎬 ІДЕЯ:\n🪝 Гачок (перші 3 секунди):\n\n"
        "🗣 ВАРІАНТ А — ГОВОРИТИ В КАМЕРУ\n"
        "Готовий розмовний текст, який можна просто зачитати в магазині (жива усна мова, короткі фрази, "
        "20-40 секунд). Повний текст, готовий до зйомки.\n\n"
        "📱 ВАРІАНТ Б — ЗЙОМКА БЕЗ СЕБЕ\n"
        "Кадри товару/процесу в магазині або на обʼєкті, телефон вертикально, що показати по кадрах + "
        "текст на екран.\n\n"
        "🎵 Музика (з бібліотеки Facebook/Instagram):\n"
...
def generate_post(theme, stage):
    ...
    prompt = (
        f"Ти ведеш сторінку магазину «{SHOP_NAME}» ({CITY}). Асортимент: {ASSORTMENT}. "
        "Згадуй лише ці товари. Пишеш для людей, що будують/ремонтують, просто, на «ви». "
        "Це КОРОТКА підтримка до відео дня.\n\n"
        + instr +
        "\nКОРОТКО, конкретно, по користі." + ACCURACY + HOOK + ENGAGE + ANTISHABLON +
        "\nВАЖЛИВО: саме цей конкретний кут. Обсяг 300-550 символів. 3-4 хештеги (будівництво, Мукачево). "
...
def make_image_prompt(theme):
    try:
        scene = ask_gemini(
            "Опиши ОДНУ коротку англійською реалістичну сцену (макс 14 слів) для поста магазину "
            f"будматеріалів на тему: «{theme}». Будівництво, матеріали, чисто. Без тексту й логотипів. "
...
def main():
    ...
    # Пост — коротка підтримка з картинкою
    time.sleep(2)
